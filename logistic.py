...
best_lambda = 0.000001 * (5 ** lambdas[np.argmax(lambdas, axis = 0)[1]][0])
...
plt.title("Learning curve with lambda = %s" %(best_lambda))
...
#find thetas
w = W0.eval(session=sess)
b = b0.eval(session=sess)

xparams = {}
xparams['w'] = w
xparams['b'] = b
pickle.dump(xparams, open("wb.pkl", "wb"))

w = pickle.load(open('wb.pkl', 'rb'))['w']

wneg = w.T[0]
wpos = w.T[1]
neg_100 = wneg.argsort()[-100:][::-1]
pos_100 = wpos.argsort()[-100:][::-1]

all_words = array(all_words)
words_neg = all_words[neg_100]
words_pos = all_words[pos_100]
